...
import org.jhipster.health.repository.repositoryImpl.BloodPressureRepositoryImpl;
...
public class BloodPressureResource {
    ...
    private final BloodPressureRepositoryImpl bloodPressureRepositoryImpl;

    public BloodPressureResource(BloodPressureRepository bloodPressureRepository, BloodPressureSearchRepository bloodPressureSearchRepository, UserRepository userRepository, BloodPressureService bloodPressureService, BloodPressureRepositoryImpl bloodPressureRepositoryImpl) {
        this.bloodPressureRepository = bloodPressureRepository;
        this.bloodPressureSearchRepository = bloodPressureSearchRepository;
        this.userRepository = userRepository;
        this.bloodPressureService = bloodPressureService;
        this.bloodPressureRepositoryImpl = bloodPressureRepositoryImpl;
    }
...
//        BloodPressure result = bloodPressureRepository.save(bloodPressure);
        bloodPressureRepositoryImpl.updateBloodPressure(bloodPressure);
    ...
    /**
     * PUT  /blood-pressures : Updates an existing bloodPressure with XML payload.
     *
     * @param bloodPressureXMLPayload the bloodPressure to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated bloodPressure,
     * or with status 400 (Bad Request) if the bloodPressure is not valid,
     * or with status 500 (Internal Server Error) if the bloodPressure couldn't be updated
     */
    @PutMapping("/blood-pressures-xml/{parser}")
    @Timed
    public ResponseEntity<?> updateBloodPressureWithXML(@PathVariable String parser, @Valid @RequestBody String bloodPressureXMLPayload) {
        log.debug("REST request to update BloodPressure with XML: {}, parser: {}", bloodPressureXMLPayload, parser);

        BloodPressure bloodPressure = parser.equalsIgnoreCase("dbf") ?
            bloodPressureService.parseWithDbf(bloodPressureXMLPayload) : bloodPressureService.parseWithXif(bloodPressureXMLPayload);
    ...
    @GetMapping("/blood-pressures-unsafe-bfla")
    @Timed
    public ResponseEntity<List<BloodPressure>> getAllBloodPressuresUnsafeBfla(Pageable pageable) {
        log.debug("REST request to get a page of BloodPressures");
        Page<BloodPressure> page;
...
//        ** AFTER ***
    ...
    @DeleteMapping("/blood-pressures-unsafe-bola/{id}")
    @Timed
    public ResponseEntity<?> deleteBloodPressureUnsafeBola(@PathVariable Long id) {
        log.debug("REST request to delete BloodPressure : {}", id);
        Optional<BloodPressure> bloodPressure = bloodPressureRepository.findById(id);
...
//        ** AFTER ***
        if (!bloodPressure.isPresent()) {
            return new ResponseEntity<>("error.http.400", HttpStatus.BAD_REQUEST);
        }
        bloodPressureRepository.deleteById(id);
        bloodPressureSearchRepository.deleteById(id);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id.toString())).build();
