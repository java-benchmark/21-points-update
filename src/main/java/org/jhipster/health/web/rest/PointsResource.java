...
public class PointsResource {
    ...
    private final PointsRepositoryInterface pointsRepositoryInterfaceUnsafe;

    private final PointsRepositoryInterface pointsRepositoryInterfaceSafe;

    private final PointsRepositoryInterface pointsRepositoryInterfaceUnsafeBean;

    public PointsResource(ApplicationContext context, PointsRepository pointsRepository, PointsSearchRepository pointsSearchRepository, UserRepository userRepository, @Qualifier("unsafe") PointsRepositoryInterface pointsRepositoryInterfaceUnsafe, @Qualifier("safe") PointsRepositoryInterface pointsRepositoryInterfaceSafe) {
        this.context = context;
        this.pointsRepository = pointsRepository;
        this.pointsSearchRepository = pointsSearchRepository;
        this.userRepository = userRepository;

        this.pointsRepositoryInterfaceUnsafe = pointsRepositoryInterfaceUnsafe;
        this.pointsRepositoryInterfaceSafe = pointsRepositoryInterfaceSafe;
        this.pointsRepositoryInterfaceUnsafeBean = (PointsRepositoryInterface) context.getBean("unsafe");
    }
    ...
    public ResponseEntity<?> updatePoints(@Valid @RequestBody Points points) {
        ...
        int i = getNumber(points);
        if(i != NUMBER) {
            throw new BadRequestAlertException("Invalid meal input", ENTITY_NAME, "wrongmeal");
        }

        pointsRepositoryInterfaceSafe.updatePoints(points);
    ...
    @PutMapping("/points-unsafe")
    @Timed
    public ResponseEntity<?> updatePointsUnsafe(@Valid @RequestBody Points points) {
        log.debug("REST request to update Points : {}", points);
        if (points.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        ...
        pointsRepositoryInterfaceUnsafe.updatePoints(points);
        pointsSearchRepository.save(points);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, points.getId().toString()))
            .body(points);
    ...
    @PutMapping("/points-unsafe-bean")
    @Timed
    public ResponseEntity<?> updatePointsUnsafeBean(@Valid @RequestBody Points points) {
        log.debug("REST request to update Points : {}", points);
        if (points.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        ...
        pointsRepositoryInterfaceUnsafeBean.updatePoints(points);
        pointsSearchRepository.save(points);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, points.getId().toString()))
            .body(points);
    ...
    private int getNumber(Points points) {
        return 1 + points.getMeals();
    }
